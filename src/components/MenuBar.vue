        ...
        <ul class="nav navbar-nav">
          ...
          <li class="categories dropdown">
            <a class="current-category dropdown-toggle"
              ...
              全部
              <span class="count badge">{{doFilterByCateId(-1)}}</span>
            </a>
            <ul class="dropdown-menu">
              <li class="total"
              ...
              <li @click="doFilterByCateId(0)">
                <a>
                  工作
                  <span class="count badge">{{doFilterByCateId(0)}}</span>
                </a>
              </li>
              ...
              <li @click="doFilterByCateId(2)">
                <a>
                  学习
                  <span class="count badge">{{doFilterByCateId(2)}}</span>
                </a>
              </li>
            </ul>
          </li>
...
<script lang="ts">
    ...
    export default class MenuBar extends Vue{
      newMemo:ItemData = new ItemData(-1,0)

      addMessage():void{
        // this['$store'].state.transMemo = new ItemData(-1,0)
        // (this as any).$store.state.isShow = true;
        this['$store'].commit('initNewMemo',this.newMemo);
      }

      doFilterByCateId(cid:number):number{
        this['$store'].state.filterIndex = cid;
        if(cid == -1){
